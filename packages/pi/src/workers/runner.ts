...
export class WorkerRunner {
  ...
  #start(item: Scheduled, writer: boolean) {
    ...
    item.work(item.signal).then((value) => { if (item.signal?.aborted) item.reject(new Error("worker cancelled")); else item.resolve(value); }, item.reject).finally(() => { if (writer) this.#activeWriters--; else this.#readers--; this.#drain(); });
  }
}
export class PiRpcRunner {
  ...
  #terminal?: { resolve: (value: string) => void; reject: (error: Error) => void; timer: NodeJS.Timeout };
  #exit: Promise<void>;
  constructor(options: { node?: string; cli: string; extension: string; cwd: string; profile: string; auth?: unknown }) {
    ...
    this.#child = spawn(options.node ?? process.execPath, [options.cli, "--mode", "rpc", "--offline", "--no-session", "--no-extensions", "-e", options.extension, "--no-skills", "--no-prompt-templates", "--no-themes", "--no-context-files", "--no-builtin-tools"], { cwd: options.cwd, shell: false, stdio: ["pipe", "pipe", "pipe", "pipe", "pipe"], detached: process.platform !== "win32", env: { ...process.env, PI_CODING_AGENT_DIR: options.profile, PI_CODING_AGENT_SESSION_DIR: options.profile } });
    this.#exit = new Promise((resolve) => this.#child.once("exit", () => {
      this.#fail(new Error("worker RPC closed"));
      resolve();
    }));
    ...
    this.#child.stdout?.on("data", (data) => { this.#buffer += data.toString("utf8"); if (Buffer.byteLength(this.#buffer, "utf8") > 65536 && !this.#buffer.includes("\n")) { this.#fail(new Error("worker RPC oversized output")); this.#child.kill("SIGTERM"); return; } for (;;) { const at = this.#buffer.indexOf("\n"); if (at < 0) return; const line = this.#buffer.slice(0, at); this.#buffer = this.#buffer.slice(at + 1); if (Buffer.byteLength(line, "utf8") > 65536) { this.#fail(new Error("worker RPC oversized output")); this.#child.kill("SIGTERM"); return; } try { const value = JSON.parse(line); if (value?.type === "agent_end" || (value?.type === "event" && value.event?.type === "agent_end")) { const terminal = this.#terminal; if (terminal) { this.#terminal = undefined; clearTimeout(terminal.timer); terminal.resolve(JSON.stringify(value.event ?? value).slice(0, 65536)); } continue; } const waiter = this.#waiters.get(value.id); if (waiter) { this.#waiters.delete(value.id); clearTimeout(waiter.timer); if (value?.type === "error" || value?.success === false) waiter.reject(new Error(value?.error?.message ?? value?.message ?? "worker RPC request rejected")); else waiter.resolve(value); } } catch { this.#fail(new Error("worker RPC malformed output")); this.#child.kill("SIGTERM"); return; } } });
  }
  ...
  async prompt(message: string, timeout = 30000) { if (this.#terminal) throw new Error("worker prompt already active"); const terminal = new Promise<string>((resolve, reject) => { const timer = setTimeout(() => { this.#terminal = undefined; reject(new Error("worker terminal timeout")); }, timeout); this.#terminal = { resolve, reject, timer }; }); terminal.catch(() => {}); await this.request("prompt", { message }, timeout); return await terminal; }
...
export function createWorkerExtension(mission: WorkerMission, backend: () => Promise<unknown>, auth?: any, lifetimeFd?: number) { const host = Object.freeze({ workspace: mission.workspace, mode: mission.mode, role: mission.role, backend }); return async (pi: any) => { if (auth) pi.registerProvider(auth.provider, { apiKey: auth.apiKey, headers: auth.headers, baseUrl: auth.baseUrl, models: [auth.model] }); pi.registerTool({ name: "worker_read", label: "Read authorized file", parameters: { type: "object", properties: { path: { type: "string" } }, required: ["path"], additionalProperties: false }, async execute(_id: string, input: any) { return { content: [{ type: "text", text: await readWorkerTarget(mission, input.path) }] }; } }); pi.registerTool({ name: "worker_check", label: "Run authorized check", parameters: { type: "object", properties: { argv: { type: "array", items: { type: "string" } } }, required: ["argv"], additionalProperties: false }, async execute(_id: string, input: any, signal?: AbortSignal) { return { content: [{ type: "text", text: await runWorkerCheck(mission, validateWorkerArgv(mission, input.argv), signal, lifetimeFd) }] }; } }); if (workerCanWrite(mission.role)) { const { createApplyPatchTool } = await import("../tools/apply_patch.ts"); pi.registerTool(createApplyPatchTool(host as any, { workerRole: mission.role, allowedTargets: workerTargetLedger(mission), onWorkerWrite: async (targets) => await advanceWorkerTargets(mission, targets), ...(mission.role === "sdd-apply" ? { acceptedBindings: mission.acceptedBindings } : {}) })); } }; }
export async function executePiWorker(mission: WorkerMission, options: { cli: string; runnerModule: string; timeout?: number; signal?: AbortSignal; auth?: unknown }) { if (process.platform === "win32") throw new Error("worker process tree ownership is unsupported on Windows"); const root = await mkdtemp(join(tmpdir(), "vgxness-pi-worker-")); let rpc: PiRpcRunner | undefined; let recovered = false; try { const extension = join(root, "worker.mjs"); await writeFile(extension, `import { readFileSync } from "node:fs"; import { createWorkerExtension, bootstrapWorkerMission } from ${JSON.stringify(options.runnerModule)}; const mission = await bootstrapWorkerMission(${JSON.stringify(mission)}); const auth = JSON.parse(readFileSync(3, "utf8")); export default createWorkerExtension(mission, async () => { throw new Error("worker backend unavailable"); }, auth, 4);\n`); rpc = new PiRpcRunner({ cli: options.cli, extension, cwd: mission.workspace, profile: join(root, "profile"), auth: options.auth }); const cancelled = new Promise<never>((_, reject) => options.signal?.addEventListener("abort", () => { void rpc?.abort().catch(() => {}); reject(new Error("worker cancelled")); }, { once: true })); if (options.signal?.aborted) throw new Error("worker cancelled"); await rpc.request("get_state"); await rpc.select(mission.model, mission.effort); return (await Promise.race([rpc.prompt(mission.goal, options.timeout), cancelled])).slice(0, mission.resultLimit); } finally { try { await rpc?.stop(); recovered = true; } finally { if (recovered) await rm(root, { recursive: true, force: true }); } } }
