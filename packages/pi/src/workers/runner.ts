...
/** Explicit worker extension surface: no ambient resources, task, or memory tools. */
...
export function createWorkerExtension(mission: WorkerMission, backend: () => Promise<unknown>, auth?: any, lifetimeFd?: number, authority?: WorkerAuthority) { const writable = mission.mode === "full" && workerCanWrite(mission.role); const mutationGuard = writable ? (authority ? createWorkerMutationGuard(authority) : (() => { throw new Error("worker mutation authority unavailable"); })) : undefined; const host = Object.freeze({ workspace: mission.workspace, mode: mission.mode, role: mission.role, backend, mutationGuard }); return async (pi: any) => { for (const tool of createExplorerTools(mission)) pi.registerTool(tool); if (auth) pi.registerProvider(auth.provider, { apiKey: auth.apiKey, headers: auth.headers, baseUrl: auth.baseUrl, models: [auth.model] }); pi.registerTool({ name: "worker_read", label: "Read authorized file", description: "Read one existing file using its exact workspace-relative key from the authorized targets ledger. Do not convert it to an absolute path.", parameters: { type: "object", properties: { path: { type: "string", description: "Exact authorized targets key, such as note.txt or src/file.ts. Workspace-relative only; do not prepend the workspace path or change the key." } }, required: ["path"], additionalProperties: false }, async execute(_id: string, input: any) { if (!input || Object.keys(input).some(key => key !== "path") || typeof input.path !== "string") throw new Error("invalid worker read input"); const text = await readWorkerTarget(mission, input.path); if (Buffer.byteLength(text) > mission.resultLimit) throw new Error("worker read exceeds result budget; use explorer pages"); return { content: [{ type: "text", text }] }; } }); if (mission.role !== "explore") pi.registerTool({ name: "worker_check", label: "Run authorized check", parameters: { type: "object", properties: { argv: { type: "array", items: { type: "string" } } }, required: ["argv"], additionalProperties: false }, async execute(_id: string, input: any, signal?: AbortSignal) { if (!input || Object.keys(input).some(key => key !== "argv")) throw new Error("invalid worker check input"); mutationGuard?.(); return { content: [{ type: "text", text: await runWorkerCheck(mission, validateWorkerArgv(mission, input.argv), signal, lifetimeFd, mutationGuard) }] }; } }); if (writable) { const { createApplyPatchTool } = await import("../tools/apply_patch.ts"); pi.registerTool(createApplyPatchTool(host as any, { workerRole: mission.role as "general", allowedTargets: workerTargetLedger(mission), onWorkerWrite: async (targets) => await advanceWorkerTargets(mission, targets) })); } }; }
