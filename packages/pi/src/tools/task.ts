...
export function createTaskTool(host: { role: string; mode?: "full" | "read-only"; workspace: string; mutationGuard?: () => void; mutationSignal?: () => AbortSignal; mutationGrant?: () => MutationGrant; resolveSkills?: typeof snapshotWorkerSkills; runner?: WorkerRunner; executeWorker?: (mission: WorkerMission, signal?: AbortSignal, auth?: unknown, authority?: Readonly<{ expiresAt: number }>) => Promise<string>; prepareWorker?: (mission: WorkerMission) => Promise<unknown>; configuredModel?: (role: string) => { model: string; effort: string } | undefined; supportsModel?: (model: string, effort: string) => boolean; verifyAcceptedBinding?: (binding: any) => Promise<boolean> }) {
  ...
  return {
    ...
    async execute(_id: string, input: any, signal?: AbortSignal) {
      ...
      const configured = host.configuredModel?.(role);
      if (configured && (configured.model !== input.model || configured.effort !== input.effort)) throw new Error("worker model differs from user configuration");
