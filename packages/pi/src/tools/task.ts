...
export function createTaskTool(host: { role: string; mode?: "full" | "read-only"; workspace: string; mutationGuard?: () => void; mutationSignal?: () => AbortSignal; mutationGrant?: () => MutationGrant; resolveSkills?: typeof snapshotWorkerSkills; runner?: WorkerRunner; executeWorker?: (mission: WorkerMission, signal?: AbortSignal, auth?: unknown, authority?: Readonly<{ expiresAt: number }>) => Promise<string>; prepareWorker?: (mission: WorkerMission) => Promise<unknown>; supportsModel?: (model: string, effort: string) => boolean; verifyAcceptedBinding?: (binding: any) => Promise<boolean> }) {
  ...
  return {
    ...
    async execute(_id: string, input: any, signal?: AbortSignal) {
      ...
      const unavailable = (detail: string) => { throw new WorkerResultError(workerTerminalEnvelope({ nonce: input.nonce, role, mode: input.mode, scope: Object.keys(input.targets ?? {}), skills: input.skills, state: "unavailable", reasonCode: "unavailable", detail, startedAt: started, model: input.model, effort: input.effort, resultLimit: input.resultLimit })); };
      ...
      }, combined); const report = (()=>{try{return JSON.parse(result)}catch{return {text:result}}})(); const terminal=workerTerminalEnvelope({nonce:mission.nonce,role:mission.role,mode:mission.mode,scope:Object.keys(mission.targets),skills:input.skills,state:"succeeded",reasonCode:"succeeded",detail:"worker settled",startedAt:started,model:mission.model,effort:mission.effort,resultLimit:mission.resultLimit,report}); if(terminal.state!=="succeeded")throw new WorkerResultError(terminal); return { content: [{ type: "text", text: JSON.stringify(terminal) }], details:{result:terminal} };
      } catch (error) { if (error instanceof WorkerResultError || (error && typeof error === "object" && (error as any).result?.nonce === input.nonce)) throw error; const report = error && typeof error === "object" && (error as any).report && typeof (error as any).report === "object" ? (error as any).report : undefined; const unavailable=(error as any)?.code==="worker_unavailable"||report?.reasonCode==="unavailable"; const cancelled=combined?.aborted && !report?.recovery?.pending; const terminal=workerTerminalEnvelope({nonce:input.nonce,role,mode:input.mode,scope:Object.keys(input.targets??{}),skills:input.skills,state:unavailable?"unavailable":cancelled?"cancelled":"failed",reasonCode:unavailable?"unavailable":cancelled?"cancelled":report?.reasonCode??"failed",detail:error instanceof Error?error.message:"worker failed",startedAt:started,model:input.model,effort:input.effort,resultLimit:input.resultLimit,report}); throw new WorkerResultError(terminal); }
    },
  };
}
