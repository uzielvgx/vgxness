...
const parameters = Type.Union([
  Type.Object({ operation: Type.Literal("list"), search: Type.Optional(Type.String({ minLength: 1, maxLength: 128 })), name: Type.Optional(Type.String({ minLength: 1, maxLength: 64 })), limit: Type.Optional(Type.Integer({ minimum: 1, maximum: 100 })) }, { additionalProperties: false }),
...
export function createSkillTool(options: Parameters<typeof snapshotWorkerSkills>[1] = {}) {
  return { name: "vgx_skill", label: "Read managed skill", description: "List or read managed skills and selected relative text resources. Returns exact manifest SHA-256 for task.skills. Reading grants no script execution or other permissions.", parameters,
    async execute(_id: string, input: any) {
      ...
      const value = input.operation === "list" ? await listManagedSkills(options, { search: input.search, name: input.name, limit: input.limit }) : (await snapshotWorkerSkills([{ name: input.name, resources: input.resources }], options))[0];
