...
test("Pi SDK registers native tools with closed operation schemas", async () => {
  ...
  assert.deepEqual(
    ...
    ["vgx_skill", "apply_patch", "memory", "memory_forget", "memory_get", "memory_recent", "memory_save", "memory_search", "model_resolve", "question", "session_context", "session_handoff", "task", "todowrite"].sort(),
  );
});
