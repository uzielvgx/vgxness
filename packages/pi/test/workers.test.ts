...
function assertProcessNotLive(pid: number) {
...
test("native worker_check abort reaps its process group", async () => {
  ...
  const mission: any = await acceptMission(issueMission({ nonce: crypto.randomUUID(), role: "explore", workspace: root, mode: "read-only", model: "test", effort: "low", goal: "check", criteria: ["check"], commands: [[process.execPath, script, pidFile]], resultLimit: 1024, targets: {} }));
...
test("native worker_check reaps descendants after a successful command leader exit", async () => {
  ...
  const mission: any = await acceptMission(issueMission({ nonce: crypto.randomUUID(), role: "explore", workspace: root, mode: "read-only", model: "test", effort: "low", goal: "check", criteria: ["check"], commands: [[process.execPath, script, descendantFile, supervisorFile]], resultLimit: 1024, targets: {} }));
  (globalThis as any).__worker = mission;
  const wrapper = join(root, "worker.ts");
  await writeFile(wrapper, `import { createWorkerExtension } from ${JSON.stringify(join(process.cwd(), "src/workers/runner.ts"))}; export default createWorkerExtension(globalThis.__worker, async () => { throw new Error("unused"); });`);
  const loaded = await loadExtensions([wrapper], root);
...
test("executePiWorker retains its root until fd4-owned checks reap after RPC death", async () => {
  ...
  const mission: any = issueMission({ workspace: root, mode: "read-only", role: "explore", model: "test/model", effort: "low", goal: "check", criteria: ["check"], commands: [[process.execPath, command, stateFile]], resultLimit: 1024, nonce: crypto.randomUUID(), targets: {} });
  ...
  assertProcessNotLive(descendant);
  assertProcessNotLive(supervisor);
