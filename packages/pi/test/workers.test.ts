...
test("retired SDD workers are rejected before execution", async () => {
 const { nativeWorkerRole } = await import("../src/models/plan.ts");
 for (const role of ["sdd-research","sdd-proposal","sdd-spec","sdd-design","sdd-tasks","sdd-apply","apply"]) assert.throws(()=>nativeWorkerRole(role),/launchable/);
});
