...
test("native worker_check abort reaps its process group", async (t) => {
  if (process.platform === "win32") { t.skip("Pi worker processes unsupported on Windows"); return; }
...
test("native worker_check reaps descendants after a successful command leader exit", async (t) => {
  if (process.platform === "win32") { t.skip("Pi worker processes unsupported on Windows"); return; }
...
test("worker_check control EOF reaps its detached command group after abrupt RPC death", async (t) => {
  if (process.platform === "win32") { t.skip("Pi worker processes unsupported on Windows"); return; }
...
test("RPC stop kills a descendant after its TERM-handling leader exits", async (t) => {
  if (process.platform === "win32") { t.skip("Pi worker processes unsupported on Windows"); return; }
...
test("executePiWorker retains its root until fd4-owned checks reap after RPC death", async (t) => {
  if (process.platform === "win32") { t.skip("Pi worker processes unsupported on Windows"); return; }
...
test("executePiWorker reports recovery_pending and retains its root while fd4 is held", async (t) => {
  if (process.platform === "win32") { t.skip("Pi worker processes unsupported on Windows"); return; }
...
test("RPC confirms native selection and retains partial/recovery transport evidence", { skip: process.platform === "win32" ? "Pi worker process ownership unsupported on Windows" : false }, async t => {
 const root=await mkdtemp(join(tmpdir(),"pi-rpc-evidence-")); t.after(()=>rm(root,{recursive:true,force:true})); const cli=join(root,"fake.mjs");
 await writeFile(cli,`import readline from 'node:readline';let model={provider:'wrong',id:'model'},thinkingLevel='low';const out=v=>process.stdout.write(JSON.stringify(v)+'\\n');readline.createInterface({input:process.stdin}).on('line',line=>{const r=JSON.parse(line);if(r.type==='set_model'){model=r.provider==='bad'?{provider:'wrong',id:'model'}:{provider:r.provider,id:r.modelId};return out({id:r.id,type:'response',success:true})}if(r.type==='set_thinking_level'){thinkingLevel=r.level;return out({id:r.id,type:'response',success:true})}if(r.type==='get_state')return out({id:r.id,type:'response',success:true,data:{model,thinkingLevel}});out({id:r.id,type:'response',success:true});if(r.type==='prompt'){out({type:'message_end',message:{role:'assistant',content:[{type:'text',text:'✓'}],usage:{input:1}}});out({type:'message_update',message:{role:'assistant'},delta:{text:'partial'}});out({type:'tool_execution_end',isError:true,result:{content:[{type:'text',text:'PatchRecoveryError recovery_pending {"affectedPaths":["a"],"recoveryPaths":["r"]}'}]}});out({type:'agent_settled'})}});`);
 const runner=new PiRpcRunner({cli,extension:'unused',cwd:root,profile:join(root,'profile')});t.after(()=>runner.stop().catch(()=>{}));await assert.rejects(runner.select('bad/model','low'),/not confirmed/);await runner.select('fixture/model','low');await assert.rejects(runner.prompt('go',500),error=>{assert.match(String(error),/recovery_pending/);const report=(error as any).report;assert.equal(report.text,'✓partial');assert.equal(report.selectedModel,'fixture/model');assert.equal(report.effectiveModel,'unknown');assert.deepEqual(report.recovery.refs,['a','r']);return true});
});

test("executePiWorker bounds failure transport text to the mission result limit", { skip: process.platform === "win32" ? "Pi worker process ownership unsupported on Windows" : false }, async t => {
 const root=await mkdtemp(join(tmpdir(),"pi-worker-failure-bound-"));t.after(()=>rm(root,{recursive:true,force:true}));const cli=join(root,"fake.mjs"),runner=join(process.cwd(),"src/workers/runner.ts");
 await writeFile(cli,`import readline from 'node:readline';const out=v=>process.stdout.write(JSON.stringify(v)+'\\n');readline.createInterface({input:process.stdin}).on('line',line=>{const r=JSON.parse(line);if(r.type==='get_state')return out({id:r.id,type:'response',success:true,data:{model:{provider:'test',id:'model'},thinkingLevel:'low'}});out({id:r.id,type:'response',success:true});if(r.type==='prompt'){out({type:'message_update',message:{role:'assistant'},delta:{text:'abcdef'}});out({type:'tool_execution_end',isError:true,result:{content:[{type:'text',text:'recovery_pending'}]}});out({type:'agent_settled'})}});`);
 const mission:any=issueMission({workspace:root,mode:'read-only',role:'explore',model:'test/model',effort:'low',goal:'x',criteria:['x'],commands:[],resultLimit:4,nonce:crypto.randomUUID(),targets:{}});
 await assert.rejects(executePiWorker(mission,{cli,runnerModule:runner,timeout:500}),error=>{assert.equal((error as any).report.text,'abcd');assert.equal((error as any).report.truncated,true);return true});
});
