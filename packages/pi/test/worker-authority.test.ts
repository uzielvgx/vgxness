...
test("native worker rejects writes even when RPC ignores cancellation during startup", { skip: process.platform === "win32" ? "Pi worker processes unsupported on Windows" : false }, async t => {
  ...
  await writeFile(cli, `import {writeFileSync,renameSync} from "node:fs"; const publish=(path,value)=>{const temporary=path+".publishing";writeFileSync(temporary,value);renameSync(temporary,path);}; import {dirname} from "node:path"; import readline from "node:readline"; process.on("SIGTERM", () => {}); process.on("uncaughtException", error => {publish(${JSON.stringify(observed)}, String(error)); process.exit(1);}); await new Promise(resolve => setTimeout(resolve, 2200)); const extPath = process.argv[process.argv.indexOf("-e") + 1]; const {loadExtensions}=await import(${JSON.stringify(join((await piPaths()).root, "dist/core/extensions/loader.js"))}); const loaded=await loadExtensions([extPath], ${JSON.stringify(root)}); if(loaded.errors.length)throw new Error(JSON.stringify(loaded.errors)); const patch=[...loaded.extensions[0].tools.values()].map(item=>item.definition).find(tool=>tool.name==="apply_patch"); const out = value => process.stdout.write(JSON.stringify(value)+"\\n"); let attempted=false; readline.createInterface({input:process.stdin}).on("line", async line => { const req=JSON.parse(line); if(req.type === "get_state") {publish(${JSON.stringify(ready)}, JSON.stringify({pid:process.pid,root:dirname(extPath)})); return;} if(req.type === "abort" && !attempted) {attempted=true; let reason=""; try {await patch.execute("late",{patch:${JSON.stringify(patch)}});} catch(error){reason=String(error);} publish(${JSON.stringify(observed)},reason); } out({id:req.id,type:"response",success:true}); });`);
