...
test("generated contract has all roles and authority",async()=>{const c=loadManagerContract();assert.equal(c.roles.length,12);assert.equal(resolveRole(c,"worker")?.id,"general");assert.equal(resolveRole(c,"apply")?.id,"sdd-apply");assert.equal(await workerAuthority("general",c),true);assert.equal(await workerAuthority("verifier",c),false);assert.deepEqual(workerRoles,c.roles.map(role=>role.id));assert.equal(workerCanWrite("general"),true);assert.equal(workerCanWrite("sdd-apply"),true);assert.equal(workerCanWrite("verifier"),false);assert.match(resolveRole(c,"care-challenger")!.instructions,/Challenge/);});
...
test("shared model roles and portable policy are complete", () => {
 ...
 assert.equal(c.roles.find((r:any)=>r.id==="explore").modelRole,undefined);
