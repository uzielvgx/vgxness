...
OpenCode Manager62 installs 11 managed artifacts: seven agents (Manager, Explore, General, Verifier and three CARE roles), the memory lifecycle plugin, model manifest and two configuration artifacts. Codex Manager21 installs nine artifacts: AGENTS.md, six delegated profiles and two plugin lifecycle manifests. Pi uses the same six worker roles through its native TypeScript SDK/RPC adapter. MCP full mode exposes eight memory tools and no SDD tools. Installation does not prove model behavior or runtime authentication.
