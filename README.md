...
For eligible implementation tasks, manager v37 completes the clean-checkout, identity, intended-path, estimate/slice, and fresh-branch pre-write gate before branch creation and source writes; candidate validation, developmental checks, independent verification, and review occur after implementation and before delivery mutations. The v3 skill permits a dirty checkout only through explicitly reauthorized, bounded unpublished-local-slice recovery, with both current HEAD and the deterministic local branch ref equal to the verified base or predecessor OID; it never recovers remote branches or PRs. Every first remote publication uses the create-only empty-expectation lease `git push --set-upstream --force-with-lease=refs/heads/<head>: <verified-remote> refs/heads/<head>:refs/heads/<head>` rather than a normal push: it fails when the remote branch exists and never updates it. Existing remote branches and PRs remain read-only. Explicit `local-only`, `no commit`, `no push`, and `no PR` restrictions narrow that flow. This is a static OpenCode policy: command globs do not prove argument semantics or guarantee Git, GitHub, credential, hook, network, or branch-protection behavior.
