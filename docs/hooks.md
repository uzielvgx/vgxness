...
`vgxness memory hook --stdin` is a local host-only JSON lifecycle bridge, not an installed hook surface. Its single JSON request and output use `schemaVersion: 1`; unknown fields and trailing documents are rejected. It supports `--storage-root` and `--project-local` consistently with other memory commands. Start, checkpoint, and end are supported; only end finalizes a provider session. Local drafts contain host-supplied text pending final validation and are never synchronized.
