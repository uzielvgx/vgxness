...
The wizard explains and verifies the complete OpenCode setup before changing anything. Current ownership is 19 OpenCode-managed artifacts plus the separate global `skills-creator`, `stacked-pr`, `cross-platform`, `installer-lifecycle`, `agent-evaluation`, and `ci-triage` catalog; the legacy provider skill is not an active artifact.
...
- the separate global 20-file `skills-creator`, `stacked-pr`, `cross-platform`, `installer-lifecycle`, `agent-evaluation`, and `ci-triage` catalog is installed without drift; OpenCode uninstall does not own it;
