...
The wizard explains and verifies the complete OpenCode setup before changing anything. Current ownership is 19 OpenCode-managed artifacts plus the separate global `skills-creator`, `stacked-pr`, `cross-platform`, `installer-lifecycle`, `agent-evaluation`, `ci-triage`, and `security-boundary` catalog; the legacy provider skill is not an active artifact.
...
5. Install the bounded VGXNESS-owned storage plugin, `<config-dir>/vgxness/model-plan.json`, the `opencode.json` default-agent selection and bounded `<config-dir>/vgxness/default-agent.json` restoration metadata, then publish global `skills-creator`, `stacked-pr`, `cross-platform`, `installer-lifecycle`, `agent-evaluation`, `ci-triage`, and `security-boundary`.
6. Read back all managed identities and perform the live OpenCode handshake.
7. Report recovery guidance if any step fails.

The resulting 19 OpenCode artifacts are 15 agents, storage plugin v5, model-plan manifest, default-agent selection, and restoration metadata. The agents include manager v38, managed `general`, verifier, `explore`, five reviewers, and six SDD profiles. Official setup retires exact provider v1/v2/v3 bytes before publishing the global 22-file `skills-creator`, `stacked-pr`, `cross-platform`, `installer-lifecycle`, `agent-evaluation`, `ci-triage`, and `security-boundary` catalog; unknown or modified bytes block. OpenCode uninstall does not own global skills.
...
- the separate global 22-file `skills-creator`, `stacked-pr`, `cross-platform`, `installer-lifecycle`, `agent-evaluation`, `ci-triage`, and `security-boundary` catalog is installed without drift; OpenCode uninstall does not own it;
