...
The resulting 16 OpenCode artifacts are 13 agents, model-plan manifest, default-agent selection, and restoration metadata. The agents include Manager58, managed `general` v10, `explore` v4, verifier v7, three CARE v2 roles/profiles, and six SDD roles/profiles including `vgxness-sdd-apply` v7. Manager58 requires a complete Candidate Capsule for frozen, risky, verification, and SDD delegations. OpenCode immediate CARE-v1/Manager58 and OpenCode next CARE-v1/Manager57 packages are predecessor-only and can be upgraded; OpenCode v56/verifier-v6 is deeper, while one-byte or mixed-package changes remain drift. `vgxness mcp --full` exposes five memory and 13 SDD tools. Official setup publishes the global 47-file, 19-skill catalog including `memory-sync`; only exact historical `vgxness.ts` v1-v10 plugin and `vgxness-autonomous-stacked-pr` v1/v2/v3 provider-skill bytes are removable, while modified, malformed, foreign, unknown, or newer bytes block without removal. OpenCode uninstall does not own global skills.
