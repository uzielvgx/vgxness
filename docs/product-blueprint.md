...
- manager v38, 14 other model-bound agents, plugin v5, model-plan manifest, default-agent selection, restoration metadata, and the separate global seven-skill catalog;
...
Setup previews all changes, requires confirmation, installs the stable launcher and 19 exact OpenCode artifacts, retires exact provider v1/v2/v3 bytes, then publishes the global 22-file `skills-creator`, `stacked-pr`, `cross-platform`, `installer-lifecycle`, `agent-evaluation`, `ci-triage`, and `security-boundary` catalog. Unknown or modified legacy bytes block. OpenCode uninstall does not remove global skills.
