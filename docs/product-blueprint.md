...
Setup previews all changes, requires confirmation, installs the stable launcher and 19 exact OpenCode artifacts, retires exact provider v1/v2/v3 bytes, then publishes the global 20-file `skills-creator`, `stacked-pr`, `cross-platform`, `installer-lifecycle`, `agent-evaluation`, and `ci-triage` catalog. Unknown or modified legacy bytes block. OpenCode uninstall does not remove global skills.
