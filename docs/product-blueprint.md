...
| OpenCode adapter | **Implemented:** Manager v29, five read-only reviewers, six read-only SDD profiles, storage plugin v5, and a low/medium/high manifest form 14 managed artifacts. The default/current `medium` plan uses Luna Fast, Terra, and Sol slots; plan or slot changes require restart. |
