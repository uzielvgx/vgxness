...
Setup previews all changes, requires confirmation, installs the stable launcher and 19 exact OpenCode artifacts, retires exact provider v1/v2/v3 bytes, then publishes the global 18-file `skills-creator`, `stacked-pr`, `cross-platform`, and `installer-lifecycle` catalog. Unknown or modified legacy bytes block. OpenCode uninstall does not remove global skills.
