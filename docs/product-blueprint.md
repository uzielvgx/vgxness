...
Managed OpenCode v48 and generated Codex v8 use `vgxness mcp --full`, exposing five memory tools and 13 SDD tools. Their normalized memory policy is intent-triggered: search all terms first, retry any-term only when needed, retrieve exact IDs after preview, and use recent recall only for explicit recent-work, session, or compaction-recovery requests. MCP has no caller identity; host/operator permissions, user authorization, and task scope own authorization. This documentation makes no runtime-security claim.

Current delivery policy is manager v48 with global `stacked-pr` v3. The manager requires its clean pre-write gate before branch creation, source writes, or routine delivery announcements. The managed `general` v6, verifier v4, and reviewer profiles v3 are current.

## Managed projection

| Artifact | Count | Responsibility |
| --- | ---: | --- |
| Manager v48 | 1 | Orchestration and lifecycle ownership. |
