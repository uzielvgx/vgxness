...
Current delivery policy is manager v37 and independent skill v3. Before branch creation and source writes or a routine delivery announcement, v37 requires clean untracked-inclusive porcelain, repository/base/ref identity, intended paths, an estimate and slice plan, and a deterministic fresh branch; candidate identity, developmental checks, independent verification, and review are post-implementation gates before delivery mutations. A dirty checkout stops writes except for explicitly current-task reauthorized recovery of a verified unpublished local slice: current HEAD and the deterministic local branch ref must each equal the exact verified base or immediate-predecessor OID; there is no upstream, live remote head, PR, or staged change; identity is unchanged; paths are slice-only; an untracked-inclusive digest is present; size is <=800 lines; and focused checks, independent verification, and selected review repeat. All clean and recovery first publications use only `git push --set-upstream --force-with-lease=refs/heads/<head>: <verified-remote> refs/heads/<head>:refs/heads/<head>`: its empty remote-ref expectation fails if the branch exists and never overwrites or updates it. Recovery may create only a new current-task PR; existing remote branches and PRs remain read-only and never gain retroactive merge or cleanup authority. The v3 skill upgrades only exact embedded v1 and v2 bytes; modified, malformed, equal-version drifted, and newer bytes are protected.
...
The integration is installed only when manager v37, all other 14 agents, storage plugin v5, the model-plan manifest, and the autonomous stacked-PR skill match their managed identities exactly. Setup health combines:
