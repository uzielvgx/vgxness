...
Current delivery is global `git-delivery` v1, not active `stacked-pr`; it is policy-only with optional isolated worktrees and no Go/runtime writer, daemon, or durable delivery state. OpenCode current identity is CARE-v2 Manager59, immediately preceded by CARE-v2 Manager58, then CARE-v1 Manager58 and Manager57.
...
Current delivery policy is manager v59 and global `git-delivery` v1, migrated exactly from `stacked-pr` v3. Before branch creation and source writes or a routine delivery announcement, v59 requires clean untracked-inclusive porcelain, repository/base/ref identity, intended paths, an estimate and slice plan, and a deterministic fresh branch; candidate identity, developmental checks, independent verification, and review are post-implementation gates before delivery mutations. Current CARE review is strict: only static proof that the entire change is passive documentation or images with no behavior, configuration, permission, or generated-output effect qualifies for exemption; extension or location alone is insufficient. Every non-exempt implementation freezes, passes the native verifier, and completes the concrete-risk CARE matrix before terminal success; `IMPLEMENTED` is intermediate only. The matrix is standard: CARE reviewer; elevated: CARE reviewer and CARE specialist; critical: CARE reviewer, CARE specialist, and CARE challenger. A non-exempt candidate is `VERIFIED` only with same-candidate verifier and applicable CARE evidence. A dirty checkout stops writes except for explicitly current-task reauthorized recovery of a verified unpublished local slice. All first publications use only the empty-expectation create-only lease; existing remote branches and PRs remain read-only. Provider v1/v2/v3 bytes are retirement identities only, not activatable skills.
...
VGXNESS installs 17 OpenCode-managed artifacts: 13 agents (`vgxness-manager` v59; managed `general` v10; `explore` v4; verifier v7; three CARE v2 roles; and six hidden SDD profiles including `vgxness-sdd-apply` v7), the exact auto-discovered `plugins/vgxness-memory-lifecycle.ts` plugin, one non-secret model-plan manifest, `opencode.json` with the default-agent selection, and bounded restoration metadata. The plugin has no `opencode.json` plugin entry. OpenCode current is CARE-v2 Manager59; its immediate predecessor is exact CARE-v2 Manager58, followed by CARE-v1 Manager58 and CARE-v1 Manager57; OpenCode v56/verifier-v6 is deeper. SQLite schema v22 backs local provider-session drafts. The managed MCP launch command is `vgxness mcp --full`; it exposes the full read/write set of eight memory and 13 SDD tools. Read-only managed profiles receive explicit non-mutating allowlists.
...
The current SQLite schema v22 contains separate structured SDD tables, per-project sync backup intents, local-only provider-session rows, and local drafts; it does not make SDD content semantic memory or turn OpenSpec projections into canonical SQLite content. Immediately after a binary upgrade from an older supported schema, read-only opens cannot migrate: `status`, `doctor`, `setup opencode --status`, and read tools may report a storage/migration failure until one write-capable memory or SDD operation opens the database and atomically applies v22. Do not delete the database. Run the write-capable operation and rerun status; see [Native memory](memory.md#upgrade-migration-caveat).
...
- after any route, may make at most one durable memory save, only for an evidence-backed, safely assessed project decision, preference, constraint, or learning;
...
For an eligible implementation task, manager v59 automatically loads global `git-delivery`, migrated exactly from `stacked-pr` v3. Its clean pre-write gate—untracked-inclusive porcelain, repository/base/ref identity, intended paths, estimate and slice plan, and a fresh branch—precedes source writes and the routine delivery announcement. Slice 1 targets the original base; later slices initially target their immediate predecessor with `Depends-On`. After a predecessor merges and is proven contained in the original base, the next PR is explicitly retargeted to the original base with validated option-free tokens and exact pre/post head/base readback before required checks and merge; any premature or wrong base, head/base drift, failed check, ambiguity, or host failure stops.
