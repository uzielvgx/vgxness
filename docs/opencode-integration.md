...
`vgxness setup opencode|codex|pi|all` is the unified setup entrypoint. OpenCode and Pi use a single model for all agents or explicit per-agent choices; Codex alone uses plans. See [Choose models during installation](model-selection.md) for the screen, CLI flags, and migration behavior. `--config-dir` selects OpenCode's root and `--codex-home` selects Codex's root independently. Existing v1/v2/v3 model manifests remain readable. With no new model selection, installed choices are preserved. Model identifiers are local configuration, not evidence of authentication or runtime availability. Global skill setup and shared preflight still run once. `vgxness setup opencode` supports `--config-dir` and the model flags, then publishes global skills to the default discoverable root. Only lower-level `vgxness skills <preview|install|status|uninstall> --skills-dir PATH` supports isolated custom roots.
