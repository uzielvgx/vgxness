...
VGXNESS installs a persistent OpenCode primary agent named `vgxness-manager`, five hidden permission-scoped subagents, and a managed plugin that exposes the goal-first `vgxness_run` entrypoint plus the lower-level `vgxness_status`, `vgxness_dispatch`, and `vgxness_orchestrate` tools. Ticket-owning explorer children additionally receive `vgxness_native_read` and the optional `vgxness_codegraph` structural broker; implementers receive ticket-bound read, edit, and Go validation brokers; the maintainer receives the same bounded brokers only through the independent self-repair channel. OpenCode remains the user interface and native subagent runtime; VGXNESS remains the authority for prompt identity, permissions, routing, deterministic waves, bounded coordination, evidence, memory decisions, and durable state.
...
- `~/.config/opencode/agents/vgxness-maintainer.md` — isolated diagnosis and repair of the VGXNESS control plane through an emergency ticket that remains independent of normal continuity and dispatch leases.
...
- The manager permits only user questions, managed VGXNESS tools, and the built-in Task tool restricted to `vgxness-explorer`, `vgxness-implementer`, and `vgxness-reviewer`. The plugin, not the manager, opens `vgxness-maintainer` only after the independent repair ticket is prepared. Child-only claim, completion, read, edit, and structural-intelligence tools remain denied to the manager.
...
Every call is bound to the ticket workspace, limited to 30 seconds and 512 KiB of output, and recorded as input/output digests plus timestamps in the durable ticket. A ticket permits at most 32 structural queries. Exhaustion returns `available=false`, `reason=query-budget-exhausted`, and explicit used/remaining counts instead of a generic denial; the managed agent stops querying and completes from collected evidence. Lifecycle and administrative commands—including install, upgrade, index, uninit, and uninstall—are not exposed. `impact` and `affected` depth is capped at five; `explore` is capped at twelve source files; oversized numeric hints are clamped to those safe limits instead of failing the whole inspection. Affected paths must stay local and pass the sensitive-path policy. The managed plugin serializes CodeGraph queries, native reads, and completion on one per-ticket lane, caps a child at sixteen total broker calls, and instructs it to finish evidence gathering by fourteen so durable completion retains two steps. The control plane also waits briefly for cross-process document locks, so valid concurrent tool emission becomes deterministic sequencing instead of a misleading policy denial.
...
Visible orchestration preserves a bounded preparation-stage diagnostic when dispatch cannot start. In particular, a dirty canonical checkout is reported as `native ticket preparation denied: source worktree is not clean`; filenames, Git output, provider stderr, prompts, and other workspace details remain excluded. Other denials, unavailable dependencies, interrupted requests, invalid responses, and internal failures retain similarly bounded classifications so the manager can choose a safe recovery without exposing sensitive data.

The implementer has no direct edit tool. It can discover paths while its ticket is active, read exact UTF-8 content through `vgxness_native_read`, and call `vgxness_native_edit` sequentially. Every successful existing-file read returns the SHA-256 of the complete file, including partial paged reads; a missing creation target returns no digest. The edit broker replaces or deletes one regular file of at most 256 KiB only when the caller supplies that latest full-file SHA-256 (or the SHA-256 from the preceding edit receipt). Deletion additionally requires `delete=true` with empty content. A new file requires `create=true` and an already existing parent directory. Each ticket permits at most 32 edits. Sensitive paths, stale digests, traversal, symlink or hard-link aliases, submodule boundaries, missing parents, rename, permission change, and non-text content are denied.

Every accepted edit is atomic and its path, old digest, new digest, byte count, creation state, and deletion state are persisted before the broker responds. Completion rechecks the worktree identity and base commit, rejects staged or direct out-of-broker changes, verifies every final file or absence against the last broker receipt, and publishes an authoritative manifest digest plus the isolated worktree path through the native response and orchestration join.
...
`repair-system` is restricted to the canonical `github.com/vgxness/vgxness` Git repository and is invoked only for a concrete failure in VGXNESS itself. Its preparation path intentionally does not open normal continuity, memory retrieval, Registry, Gatekeeper, Chronicle, the native coordinator, or the regular exclusive dispatch lease. It snapshots committed `HEAD` into an isolated sibling worktree even when the source checkout is dirty, so a failed normal ticket cannot prevent diagnosis or correction.

The `vgxness-maintainer` receives only ticket-authenticated read, edit, delete, and closed validation brokers. It cannot use shell, direct filesystem tools, Git, network, delegation, commits, pushes, or automatic integration. Every repair edit invalidates prior validation receipts. A successful completion requires at least one bounded edit plus successful `test` and `vet` receipts collected after the latest edit, then emits the same explicit content manifest and isolated worktree artifact used by the normal delivery lifecycle. A schema-valid blocked or follow-up outcome is preserved without an artifact and its partial emergency worktree is removed.
