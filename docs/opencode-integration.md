...
OpenCode owns 16 managed artifacts: 13 agents, a model-plan manifest, an `opencode.json` default-agent selection, and restoration metadata. The separate global 47-file, 19-skill portable catalog adds `memory-sync` and `sdd-lifecycle`; exact historical `vgxness.ts` v1-v10 plugin bytes and `vgxness-autonomous-stacked-pr` v1/v2/v3 provider-skill bytes are removable, while modified, malformed, foreign, unknown, or newer bytes block without removal.
...
2. all 16 OpenCode-managed artifacts are installed without drift: 13 agents, model-plan manifest, default-agent selection, and restoration metadata;
