...
Preview and status are read-only. The managed agent catalogue contains manager v52 plus 14 other profiles; `general` is v9, `vgxness-sdd-apply` is v6, verifier v6, and reviewers are v4 except reliability v5. Exact historical manager v51 and all older recognized manager, agent, model-plan, and `vgxness.ts` v1-v10 plugin artifacts remain predecessor identities; modified or unknown bytes are drift and block replacement. Exact provider `vgxness-autonomous-stacked-pr` v1/v2/v3 bytes are removable; modified, malformed, foreign, unknown, or newer bytes block without removal. Uninstall removes only exact provider artifacts and never removes global skills.
...
Research, proposal, spec, design, and tasks SDD profiles are read-only. `vgxness-sdd-apply` v6 alone has workspace-write authority for an explicitly accepted, hash-bound SDD apply or OpenSpec/hybrid projection; it cannot ask questions, delegate, persist memory, save or accept revisions, record projections, or transition lifecycle state. `general` v9 may write only ordinary authorized non-SDD repository implementation and rejects SDD missions. Manager owns lifecycle state, projections, and transitions but is not the workspace writer; verifier remains non-mutating.
...
The integration is installed only when manager v52, all other 14 agents (including `general` v9, `vgxness-sdd-apply` v6, verifier v6, and reviewers v4 except reliability v5), the model-plan manifest, default-agent selection, and restoration metadata match their provider identities exactly. Setup health combines:
