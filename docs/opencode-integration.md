...
Current delivery is global `git-delivery` v1, not active `stacked-pr`; it is policy-only with optional isolated worktrees and no Go/runtime writer, daemon, or durable delivery state. OpenCode current identity is CARE-v2 Manager60, immediately preceded by Manager59, then CARE-v2 Manager58 and CARE-v1 Manager58/Manager57.
...
VGXNESS installs 17 OpenCode-managed artifacts: 13 agents (`vgxness-manager` v60; managed `general` v10; `explore` v4; verifier v7; three CARE v2 roles; and six hidden SDD profiles including `vgxness-sdd-apply` v7), the exact auto-discovered `plugins/vgxness-memory-lifecycle.ts` plugin, one non-secret model-plan manifest, `opencode.json` with the default-agent selection, and bounded restoration metadata. The plugin has no `opencode.json` plugin entry. OpenCode current is CARE-v2 Manager60; its immediate predecessor is exact Manager59, followed by CARE-v2 Manager58 and CARE-v1 Manager58/Manager57; OpenCode v56/verifier-v6 is deeper. Manager60 uses the shared pedagogical Execution Brief contract for non-trivial work. SQLite schema v23 backs local provider-session drafts and leases. The managed MCP launch command is `vgxness mcp --full`; it exposes the full read/write set of eight memory and 13 SDD tools. Read-only managed profiles receive explicit non-mutating allowlists.
