...
| `internal/providers/opencode` | Manager v58, 12 other model-bound agents (`general` v10, `explore` v4, verifier v7, three CARE v1 roles, and six SDD roles including `sdd-apply` v7), model-plan manifest, exact manager-v57 immediate predecessor, manager-v56/verifier-v6 deeper lifecycle identity and older managed predecessors, historical plugin v1–v10 and provider-skill v1/v2/v3 retirement identities, sync plumbing, and the setup handshake. The separate 47-file, 19-skill catalog includes `memory-sync` and `sdd-lifecycle`, the latter loaded only after explicit SDD acceptance. |
| `internal/providers/codex` | Standalone Codex manager v17 projection for `AGENTS.md` and 12 delegated profiles, with the exact Codex manager v16 artifact validated as its immediate historical predecessor, Codex manager v15 retained as a deeper lifecycle identity, and exact `low`, `medium`, `high`, and `ultra` model-plan projections while preserving user-owned `config.toml`. |
...
- manager v58 with global tool permission, adaptive zero-ceremony fast paths, bounded attempt budgets, and a pre-write delivery gate;
