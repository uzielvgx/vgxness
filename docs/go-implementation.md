...
| `internal/providers/opencode` | OpenCode current CARE v2 roles with Manager58, 12 other model-bound agents (`general` v10, `explore` v4, verifier v7, and six SDD roles including `sdd-apply` v7), model-plan manifest, OpenCode immediate CARE-v1/Manager58 predecessor, OpenCode next CARE-v1/Manager57 predecessor, OpenCode v56/verifier-v6 deeper lifecycle identity, historical plugin v1–v10 and provider-skill v1/v2/v3 retirement identities, sync plumbing, and the setup handshake. The separate 47-file, 19-skill catalog includes `memory-sync` and `sdd-lifecycle`, the latter loaded only after explicit SDD acceptance. |
| `internal/providers/codex` | Standalone Codex current Manager17 projection for `AGENTS.md` and 12 delegated profiles, with Codex Manager16 as immediate predecessor and Codex Manager15/v14 as deeper lifecycle identities, plus exact `low`, `medium`, `high`, and `ultra` model-plan projections while preserving user-owned `config.toml`. |
