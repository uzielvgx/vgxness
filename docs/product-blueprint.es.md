...
La política actual de entrega usa manager v55 con `stacked-pr` v3 global. El manager exige su pre-write gate limpio antes de crear ramas, escribir código o anunciar entregas rutinarias. `general` administrado v10, `explore` v4, `sdd-apply` v7, verifier v7, tres roles CARE v1 y los perfiles SDD restantes son actuales.

## Proyección administrada

| Artefacto | Cantidad | Responsabilidad |
| --- | ---: | --- |
| Manager v55 | 1 | Routing general adaptativo y autoridad del ciclo SDD cuando se activa; no escribe el workspace SDD. |
| General v10, Explore v4 y verificador v7 | 3 | General implementa trabajo autorizado no SDD; explore y verifier no mutan. |
| Perfiles CARE v1 | 3 | Revisión especializada de solo lectura. |
| Seis perfiles SDD, incluido SDD apply v7 | 6 | Roles SDD de investigación, proposal, spec, design y tasks de solo lectura, más el único escritor de workspace SDD aceptado. |
...
El paquete exacto manager-v54 es el predecesor inmediato reconocido; manager-v53/verifier-v6 y las identidades compatibles anteriores permanecen como identidades de ciclo más profundas. La documentación histórica de predecesores no describe el límite de propiedad generado actual.
