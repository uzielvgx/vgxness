...
OpenCode administrado Manager61 y Codex generado Manager20 (paridad OpenCode-v61) se renderizan desde `internal/orchestration/manager_contract.json`. Su adaptador MCP usa `vgxness mcp --full`, que expone ocho herramientas de memoria y 13 de SDD. El Manager compartido controla autorización, alcance, ciclo de vida y aceptación final; delega misiones independientes y acotadas, selecciona skills aplicables y mantiene un solo escritor. La verificación independiente y las revisiones aplicables usan el mismo candidato congelado. Los presupuestos fijos de intentos y Execution Brief pertenecen a prompts predecesores; no son enforcement actual del runtime.
...
| --- | ---: | --- |
| Manager v61 | 1 | Routing general adaptativo y autoridad del ciclo SDD cuando se activa; no escribe el workspace SDD. |
