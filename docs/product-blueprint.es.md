...
Política de entrega actual: manager v37 y skill v3 exigen un preflight limpio (identidad, paths previstos, estimación/plan de slice y branch fresca determinista) antes de crear la branch, escribir código o anunciar entrega; la identidad del candidato, checks de desarrollo, verificación independiente y review son gates posteriores a la implementación y previos a mutaciones de entrega. Solo la recuperación local no publicada, reautorizada y verificada, puede continuar desde suciedad, y tanto HEAD actual como la ref local determinista deben igualar el OID completo exacto de la base o del predecesor verificado; nunca recupera una branch o PR remota ni concede autoridad retroactiva de merge o cleanup. Toda primera publicación, limpia o de recuperación, usa exclusivamente el lease de creación con expectativa vacía `git push --set-upstream --force-with-lease=refs/heads/<head>: <verified-remote> refs/heads/<head>:refs/heads/<head>`, que falla en vez de actualizar una ref remota existente. La migración acepta exclusivamente predecesores v1 y v2 exactos.
...
- manager v37, otros 14 agentes ligados a modelos, plugin v5, manifiesto de modelos y un skill independiente de PR apiladas autónomas;
...
| --- | ---: | --- |
| Manager v37 | 1 | Única autoridad de orquestación, ciclo, Git y GitHub. |
