...
- manager v38, otros 14 agentes ligados a modelos, plugin v5, manifiesto de modelos, selección predeterminada, metadatos de restauración y la skill global `stacked-pr`;
...
Setup explica los cambios, requiere confirmación, instala launcher y 19 artefactos exactos de OpenCode, retira bytes exactos v1/v2/v3 del proveedor y luego publica el catálogo global de 20 archivos `skills-creator`, `stacked-pr`, `cross-platform`, `installer-lifecycle`, `agent-evaluation` y `ci-triage`. Bytes desconocidos o modificados bloquean. La desinstalación de OpenCode no elimina skills globales.
