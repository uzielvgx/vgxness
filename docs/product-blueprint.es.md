...
**Implemented/Partial:** Los planes de delegación validados, waves deterministas, sesiones nativas de Navigator/tareas, autoridad durable de slots y propietario/epoch, admisión condicionada a prerrequisitos, replay preparado, clasificación fail-closed, takeover de checkpoints, resultados dependientes delimitados y joins aceptados por autoridad ya impulsan `vgxness_orchestrate` desde el manager. Los flujos completos de fases/artefactos SDD, Delivery Authority, una UX de autonomía más rica, la TUI, adaptadores adicionales y el ciclo avanzado de memoria siguen planificados.
...
| **Partial** | Runtime nativo de Navigator/planes/waves/joins visible para el manager, aún delimitado a operaciones soportadas y OpenCode; continuidad amplia de planes/checkpoints/artefactos Chronicle, Delivery Authority, revisión estricta de contenido no rastreado y smoke nativo de Windows siguen incompletos. |
