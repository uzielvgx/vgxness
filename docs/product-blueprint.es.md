...
OpenCode administrado v48 y Codex generado v8 usan `vgxness mcp --full`, que expone cinco herramientas de memoria y 13 de SDD. Su política normalizada de memoria se activa por intención: busca primero todos los términos, reintenta con cualquier término solo cuando hace falta, obtiene IDs exactos después del preview y usa memoria reciente únicamente para solicitudes explícitas de trabajo reciente, sesión o recuperación de compactación. MCP no tiene identidad del llamador; los permisos del host/operador, la autorización del usuario y el alcance de tarea poseen la autorización. Esta documentación no afirma seguridad de runtime.

La política actual de entrega usa manager v48 con `stacked-pr` v3 global. El manager exige su pre-write gate limpio antes de crear ramas, escribir código o anunciar entregas rutinarias. `general` administrado v6, verifier v4 y los perfiles reviewer v3 son actuales.

## Proyección administrada

| Artefacto | Cantidad | Responsabilidad |
| --- | ---: | --- |
| Manager v48 | 1 | Orquestación y propiedad del ciclo. |
