...
- [ ] **Planned:** Add Delivery Authority, Chronicle plan-event projection, full SDD phases/artifact stores, richer approval UX, and additional adapters.
- [ ] **Planned:** Keep future `run inspect` and recovery commands focused on debugging/auditing, not happy-path ceremony.

## Next step

Use this document with the contract schemas in [`docs/schemas/README.md`](schemas/README.md) and the [native delegation/Delivery Authority implementation plan](delegation-authority-implementation-plan.md). The next major slice is content-bound Delivery Authority, followed by broader SDD artifacts and Chronicle plan events. Native Windows runtime/distribution smoke is deferred.
