...
func TestGoCIWorkflowContract(t *testing.T) {
	...
	t.Run("standard lanes are independent and aggregated", func(t *testing.T) {
		lanes := []string{"coverage", "postgres-integration", "race", "static", "vulnerability", "linux-e2e", "fuzz-launcher-manifest", "windows-compile", "windows-install", "darwin-smoke", "pi-typescript"}
		for _, lane := range lanes {
			if !strings.Contains(workflow, "  "+lane+":\n") {
				t.Errorf("workflow missing standard lane %q", lane)
			}
		}
		if strings.Count(workflow, "    needs:") != 1 || !strings.Contains(workflow, "needs: [coverage, postgres-integration, race, static, vulnerability, linux-e2e, fuzz-launcher-manifest, windows-compile, windows-install, darwin-smoke, pi-typescript]") {
	...
	t.Run("all standard evidence is declared", func(t *testing.T) {
		for _, command := range []string{
			...
			"go test -tags=e2e -count=1 -run '^TestCleanCheckoutSetupAndMemory$' ./internal/e2e",
...
func assertOpenCodeDocumentationContract(t *testing.T) {
	...
	documents := map[string][]string{
		...
		"../../docs/go-implementation.md":     {"Structured SDD is retired", "OpenCode Manager62", "Codex Manager21", "same frozen candidate", "schema v23", "eight memory tools", "modified, malformed, foreign, unknown, or newer bytes block without removal"},
		"../../docs/opencode-setup-wizard.md": {"Structured SDD is retired", "OpenCode Manager62", "Codex Manager21", "same frozen candidate", "schema v23", "eight memory tools", "modified, malformed, foreign, unknown, or newer bytes block without removal"},
		"../../docs/codex-integration.md":     {"Structured SDD is retired", "OpenCode Manager62", "Codex Manager21", "same frozen candidate", "schema v23", "eight memory tools", "modified, malformed, foreign, unknown, or newer bytes block without removal"},
