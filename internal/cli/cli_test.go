...
func TestCLI_StatusDoctorHealthy(t *testing.T) {
	for _, command := range []string{"status", "doctor"} {
		t.Run(command, func(t *testing.T) {
			...
			code := Run(context.Background(), []string{command, "--storage-root", "/tmp/root"}, &out, &stderr, f)
			testutil.Require(t, code == 0, "exit=%d stderr=%s", code, stderr.String())
			testutil.Require(t, strings.Contains(out.String(), "chronicle=absent") && stderr.Len() == 0, "unexpected output: %q / %q", out.String(), stderr.String())
		})
	}
}
...
func TestCLI_RejectsUnsupportedExportDuringWriteWithoutMutation(t *testing.T) {
	f := &fakeInspector{}
	var out, stderr bytes.Buffer
	code := Run(context.Background(), []string{"export"}, &out, &stderr, f)
...
func TestCLI_RejectsV1NonGoalCommands(t *testing.T) {
	for _, command := range []string{"tui", "agent", "backup", "restore", "sync"} {
		code := Run(context.Background(), []string{command}, &bytes.Buffer{}, &bytes.Buffer{}, &fakeInspector{})
		testutil.Require(t, code == 2, "%s exit=%d", command, code)
	}
}
