...
func TestCLI_VersionDispatchesWithoutInspector(t *testing.T) {
	...
	code := Run(context.Background(), []string{"version"}, &stdout, &stderr, inspector)
...
func TestCLI_VersionRejectsExtraArgumentsWithoutStdout(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := Run(context.Background(), []string{"version", "extra"}, &stdout, &stderr, &fakeInspector{})
...
func TestCLI_GeneralUsageIncludesVersion(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := Run(context.Background(), nil, &stdout, &stderr, &fakeInspector{})
