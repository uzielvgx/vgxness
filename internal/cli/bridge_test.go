...
type fakeBridgeRuntime struct {
	...
	validation bridge.NativeValidationRequest
...
func (runtime *fakeBridgeRuntime) ValidateNative(_ context.Context, _ string, request bridge.NativeValidationRequest) (bridge.Response, error) {
...
func TestBridgeNativeLifecycleRoutesExactRequests(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"validate", `{"protocolVersion":"1","ticketId":"ticket-1","childSessionId":"ses_child","operation":"test","packages":["./internal/bridge"]}`},
	...
	if runtime.request.ParentSessionID != "ses_parent" || runtime.completion.ChildSessionID != "ses_child" || runtime.failure.Category != "native-subagent-failed" || runtime.read.Path != "go.mod" || runtime.edit.Path != "go.mod" || runtime.validation.Packages[0] != "./internal/bridge" || runtime.codegraph.Symbol != "Service.Dispatch" {
		t.Fatalf("native lifecycle was not routed: request=%#v completion=%#v failure=%#v read=%#v edit=%#v validation=%#v codegraph=%#v", runtime.request, runtime.completion, runtime.failure, runtime.read, runtime.edit, runtime.validation, runtime.codegraph)
	}
}
