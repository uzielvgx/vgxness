...
func TestSetupWizardSuccessfulApplyReportsGlobalSkillCatalog(t *testing.T) {
	...
	fake := &fakeSetupRuntime{
		...
		result: setupflow.Result{
			Plan:        resultPlan,
			SelfInstall: selfinstall.Result{LauncherPath: "/stable/vgxness"},
			Integration: integration.Result{ArtifactCount: 19, ManifestPath: "/config/vgxness/model-plan.json"},
	...
	output := stdout.String()
	if code != 0 || fake.applyCalls != 1 || stderr.Len() != 0 || !strings.Contains(output, "catálogo global de 23 archivos") || !strings.Contains(output, "skills-creator + stacked-pr + cross-platform + installer-lifecycle + agent-evaluation + ci-triage + security-boundary + documentation-strategy + product-requirements + software-architecture-docs + user-documentation + api-documentation + quality-test-documentation + operations-runbooks + governance-compliance-docs + release-lifecycle-docs + end-to-end-testing") {
