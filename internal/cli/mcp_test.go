...
func TestRunMCPDispatchesOnceWithWorkspaceAndConfig(t *testing.T) {
	...
	code := runMCP(context.Background(), []string{"--storage-root", "/store", "--project-local"}, strings.NewReader("request"), &bytes.Buffer{}, &bytes.Buffer{}, "/workspace", func(_ context.Context, workspace string, options config.Options) error {
		called++
...
func TestRunMCPRejectsArgumentsAndReportsStartupFailureToStderr(t *testing.T) {
	for _, tc := range []struct {
		...
		called := 0
		code := runMCP(context.Background(), tc.args, strings.NewReader(""), &stdout, &stderr, "/workspace", func(context.Context, string, config.Options) error {
			called++
			return tc.err
		})
