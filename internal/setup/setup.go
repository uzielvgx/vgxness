...
func OpenCodeSteps() []Step {
	return []Step{
		...
		{Number: 3, Title: "Instalar manager y subagentes nativos", Explanation: "Crearé vgxness-manager y los perfiles ocultos vgxness-explorer, vgxness-implementer y vgxness-reviewer. Cada subagente recibe permisos propios y no puede volver a delegar.", Mutates: true},
		{Number: 4, Title: "Instalar el plugin y bridge acotado", Explanation: "Crearé vgxness_status y vgxness_dispatch en un plugin administrado. Cada dispatch prepara un ticket en VGXNESS, ejecuta una sesión hija nativa de OpenCode y acepta su resultado antes de publicar recibos, memoria o cápsulas; no inicia un segundo proceso OpenCode. El modelo queda fijado fuera de los argumentos controlados por el agente.", Mutates: true},
