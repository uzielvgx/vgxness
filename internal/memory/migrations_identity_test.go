...
func TestProjectRepairMigrationFromV16PreservesMemory(t *testing.T) {
	...
	testutil.Require(t, err == nil && version == 18 && itemErr == nil && item.Content == "preserved", "version=%d err=%v item=%+v itemErr=%v", version, err, item, itemErr)
}
func TestProjectIdentityMigrationFromV12Fixture(t *testing.T) {
	...
	defer store.Close()
	version, err := store.Health(context.Background())
	testutil.Require(t, err == nil && version == 18, "version=%d err=%v", version, err)
}
...
func TestProjectCursorMigrationFromV15Fixture(t *testing.T) {
	...
	testutil.Require(t, err == nil && version == 18 && cursor == 0 && inbox == 0, "version=%d cursor=%d inbox=%d err=%v", version, cursor, inbox, err)
}
...
func TestSyncPortableIdentityAdoptionMigrationFromV14PreservesMappings(t *testing.T) {
	...
	testutil.Require(t, err == nil && version == 18 && mappings == 1 && adoptions == 0 && inverseErr == nil && found && inverse == local && translateErr == nil && len(translated) == 1 && translated[0].RecordID == wire, "version=%d mappings=%d adoptions=%d inverse=%q/%t/%v translated=%+v err=%v", version, mappings, adoptions, inverse, found, inverseErr, translated, err)
}
