...
func TestMemoryRuntime_LiteralV1UpgradeRestartPreservesDataAndTitles(t *testing.T) {
	...
	testutil.Require(t, err == nil && version == 19, "health=%d %v", version, err)
}
...
func TestSyncEnrollmentRecoveryMigrationAndMislabeledSchema(t *testing.T) {
	...
	testutil.Require(t, err == nil && found && profile.CredentialRef == "secret://keychain/legacy" && profile.PreviousCredentialRef == "" && healthErr == nil && version == 19, "profile=%+v found=%t errors=%v/%v version=%d", profile, found, err, healthErr, version)
	testutil.NoError(t, store.Close())
	db, err = sql.Open("sqlite", path)
	testutil.NoError(t, err)
...
func TestEnvironmentIsolation_NoAmbientHomeOrNetwork(t *testing.T) {
	...
	store := openTestStore(t)
	version, err := store.Health(context.Background())
	testutil.Require(t, err == nil && version == 19, "isolated health=%d %v", version, err)
}
...
func TestOpen_ConcurrentFreshProcesses(t *testing.T) {
	...
	version, err := HealthFile(context.Background(), path)
	testutil.Require(t, err == nil && version == 19, "concurrent health=%d err=%v", version, err)
}
