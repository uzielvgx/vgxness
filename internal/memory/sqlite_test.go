...
func TestMemoryRuntime_LiteralV1UpgradeRestartPreservesDataAndTitles(t *testing.T) {
	...
	testutil.Require(t, err == nil && version == 14, "health=%d %v", version, err)
}
...
func TestSyncEnrollmentRecoveryMigrationAndMislabeledSchema(t *testing.T) {
	...
	db, err := sql.Open("sqlite", path)
	testutil.NoError(t, err)
	_, err = db.Exec(`ALTER TABLE sync_profiles DROP COLUMN previous_credential_ref; DROP TABLE sync_portable_identities; DROP TABLE portable_project_identities; PRAGMA user_version=11`)
	testutil.NoError(t, err)
	testutil.NoError(t, db.Close())
	store = openPath(t, path)
	profile, found, err := store.GetSyncProfile(context.Background())
	version, healthErr := store.Health(context.Background())
	testutil.Require(t, err == nil && found && profile.CredentialRef == "secret://keychain/legacy" && profile.PreviousCredentialRef == "" && healthErr == nil && version == 14, "profile=%+v found=%t errors=%v/%v version=%d", profile, found, err, healthErr, version)
	testutil.NoError(t, store.Close())
	db, err = sql.Open("sqlite", path)
	testutil.NoError(t, err)
	_, err = db.Exec(`ALTER TABLE sync_profiles DROP COLUMN previous_credential_ref; DROP TABLE sync_portable_identities; PRAGMA user_version=13`)
	testutil.NoError(t, err)
	testutil.NoError(t, db.Close())
	store, err = Open(context.Background(), path, nil)
	if store != nil {
...
func TestEnvironmentIsolation_NoAmbientHomeOrNetwork(t *testing.T) {
	...
	store := openTestStore(t)
	version, err := store.Health(context.Background())
	testutil.Require(t, err == nil && version == 14, "isolated health=%d %v", version, err)
}
...
func TestHealthFile_HealthyDatabaseWithoutMutation(t *testing.T) {
	...
	testutil.Require(t, err == nil && version == 14, "health=%d err=%v", version, err)
...
func TestOpen_ConcurrentFreshProcesses(t *testing.T) {
	...
	version, err := HealthFile(context.Background(), path)
	testutil.Require(t, err == nil && version == 14, "concurrent health=%d err=%v", version, err)
}
