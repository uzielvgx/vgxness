...
// Native file reads and edits remain fail-closed on platforms without the Unix
// link-count boundary used by the first supported broker implementation.
func nativeSingleLink(os.FileInfo) bool { return false }

// Native filesystem brokers are initially supported only where a stable device/inode
