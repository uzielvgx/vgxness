...
func TestSetupRendersSuccessAndFailureRecovery(t *testing.T) {
	...
	for _, expected := range []string{"SETUP COMPLETE", "changed  yes", "/bin/vgxness", "artifacts  15", "healthy", "Restart OpenCode"} {
...
func readySetupPlan(plan string) SetupPlan {
	...
	return SetupPlan{
		...
		ArtifactCount: 15, HandshakeOK: true, HandshakeStatus: "healthy", ModelPlan: plan,
...
func successfulSetupResult() SetupResult {
	return SetupResult{
		...
		ArtifactCount: 15, HandshakeOK: true, HandshakeStatus: "healthy", RestartRequired: true,
	}
}
