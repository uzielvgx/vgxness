...
func TestVersionEvolutionImmediatePredecessorIsExactCAREV2ManagerV58Package(t *testing.T) {
	current, err := buildModelPlanBundle(sdd.DefaultModelPlanConfig())
	testutil.NoError(t, err)
	predecessor, err := immediatePredecessor(current)
	testutil.NoError(t, err)
	if !bytes.Equal(predecessor.agents[managerAgentName], mustManagerV58(t, current)) || !bytes.Contains(current.agents[managerAgentName], []byte("version: 59")) || !bytes.Contains(predecessor.agents[managerAgentName], []byte("version: 58")) {
		t.Fatal("immediate predecessor did not retain exact Manager58 artifact identity")
	}
...
func mustManagerV58(t *testing.T, current modelPlanBundle) []byte {
	t.Helper()
...
func TestCurrentManagerAnchorValidationRejectsAbsentAndDuplicateAnchors(t *testing.T) {
	for name, prompt := range map[string][]byte{
		"absent assurance":          bytes.Replace([]byte(previousManagerPromptV57), []byte(currentManagerAssurance), nil, 1),
		"duplicate assurance":       append([]byte(previousManagerPromptV57), []byte(currentManagerAssurance)...),
		"absent review":             bytes.Replace([]byte(previousManagerPromptV57), []byte(managerReviewDepthV56Previous), nil, 1),
		"duplicate review":          append([]byte(previousManagerPromptV57), []byte(managerReviewDepthV56Previous)...),
		"absent refuter route":      bytes.Replace([]byte(previousManagerPromptV57), []byte(historicalOpenCodeRefuterRouting), nil, 1),
		"duplicate refuter route":   append([]byte(previousManagerPromptV57), []byte(historicalOpenCodeRefuterRouting)...),
		"absent refuter binding":    bytes.Replace([]byte(previousManagerPromptV57), []byte(historicalOpenCodeRefuterBinding), nil, 1),
		"duplicate refuter binding": append([]byte(previousManagerPromptV57), []byte(historicalOpenCodeRefuterBinding)...),
		"absent refuter handoff":    bytes.Replace([]byte(previousManagerPromptV57), []byte(historicalOpenCodeRefuterHandoff), nil, 1),
		"duplicate refuter handoff": append([]byte(previousManagerPromptV57), []byte(historicalOpenCodeRefuterHandoff)...),
	} {
...
func TestV54PreservesV53ThenV51ReliabilityPredecessorsAndSkillReceipts(t *testing.T) {
	...
	for _, required := range []string{"Before candidate inspection", "receipt naming it and status loaded|unavailable", "missing/unavailable is INCONCLUSIVE"} {
		if !bytes.Contains(historical.agents[reviewReliabilityName], []byte(required)) {
			t.Errorf("reliability contract missing %q", required)
		}
	}
	v54, err := previousV54ModelPlanBundle(historical)
	testutil.NoError(t, err)
	predecessor, err := previousV53ModelPlanBundle(v54)
	testutil.NoError(t, err)
...
func TestV54UsesCompactProtocolAndReconstructsCompleteV45Bundle(t *testing.T) {
	...
	for _, required := range []string{"version: 59", "Mission Instance v1", "Candidate Capsule v1", "Child Return Envelope v1", "Evidence Receipt v1", "8 KiB", "16 KiB", "verificationState"} {
		if !bytes.Contains(current.agents[managerAgentName], []byte(required)) {
			t.Errorf("manager missing compact protocol %q", required)
		}
	}
	v45, err := previousV45ModelPlanBundle(historical)
	testutil.NoError(t, err)
	if got, minimum := len(predecessorBundlesMust(t, historical)), 60; got < minimum {
	...
	for _, name := range compactProtocolAgentNames {
		if !bytes.Contains(v43.agents[name], []byte("version: 2")) || bytes.Equal(v43.agents[name], historical.agents[name]) {
...
func TestManagerPromptKeepsDeliveryAuthorityWithinStaticBudget(t *testing.T) {
	...
	for _, required := range []string{
		...
		"automatically load `git-delivery`",
...
func TestReadinessV54ManagerIsAdaptiveWithoutNegativeCeremonyAndRecognizesV53ThenV52(t *testing.T) {
	...
	if len(predecessors) < 10 || !bytes.Contains(predecessors[0], []byte(managerV58Marker)) || !bytes.Contains(predecessors[1], []byte(managerV57Marker)) || !bytes.Contains(predecessors[2], []byte(managerV56Marker)) || !bytes.Contains(predecessors[3], []byte("artifact: opencode-agent/vgxness-manager; version: 55")) || !bytes.Contains(predecessors[4], []byte("artifact: opencode-agent/vgxness-manager; version: 54")) || !bytes.Contains(predecessors[5], []byte("artifact: opencode-agent/vgxness-manager; version: 53")) || !bytes.Contains(predecessors[6], []byte("artifact: opencode-agent/vgxness-manager; version: 52")) || !bytes.Contains(predecessors[6], []byte(adaptiveManagerMemoryPolicy)) || !bytes.Contains(predecessors[6], []byte("Context Capsule v1")) {
		t.Fatal("manager predecessors must begin M58, M57, M56, M55, M54, M53, then M52 with context continuity")
	}
	if !bytes.Contains(predecessors[9], []byte("artifact: opencode-agent/vgxness-manager; version: 49")) || !bytes.Contains(predecessors[9], []byte(adaptiveManagerMemoryPolicy)) || bytes.Contains(predecessors[9], []byte("Context Capsule v1")) {
		t.Fatal("exact context-free v49 predecessor with intent-triggered memory semantics is tenth")
	}
}
...
func TestVersionEvolutionUsesOnlyManagedHEADPredecessors(t *testing.T) {
	...
	testutil.NoError(t, err)
	for name, marker := range map[string]string{
		managerAgentName:  "artifact: opencode-agent/vgxness-manager; version: 59",
	...
	for name, marker := range map[string]string{reviewRiskName: "artifact: opencode-agent/vgxness-review-risk; version: 4", reviewReadabilityName: "artifact: opencode-agent/vgxness-review-readability; version: 4", reviewReliabilityName: "artifact: opencode-agent/vgxness-review-reliability; version: 5", reviewResilienceName: "artifact: opencode-agent/vgxness-review-resilience; version: 4", reviewRefuterName: "artifact: opencode-agent/vgxness-review-refuter; version: 4"} {
		if !bytes.Contains(historical.agents[name], []byte(marker)) {
			t.Errorf("historical %s missing %q", name, marker)
		}
	}
