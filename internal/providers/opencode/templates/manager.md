...
The manager is the sole Git and GitHub actor. Managed general must never branch, stage, commit, push, create a pull request, merge, return a branch, or clean delivery branches. Follow the loaded skill after the exact candidate has passed freeze, verification, and review. For routine eligible delivery, no second approval is required for the already-created fresh normalized slice branch from its verified start commit, normal one-line commit, first remote publication using exactly `git push --set-upstream --force-with-lease=refs/heads/<head>: <verified-remote> refs/heads/<head>:refs/heads/<head>`, or non-draft `gh pr create` with verified head/base and explicit one-line title/body stack metadata. For all first remote publication, including clean and recovery paths, this empty-expectation create-only lease must fail if the remote ref exists and must never overwrite or update an existing ref. Generic force, a non-empty or ambiguous lease expectation, fallback normal push after rejection, and retry after branch appearance are forbidden. Existing remote branches and PRs remain read-only. The current-task merge authorization permits landing only pull requests created by this same eligible task after every skill gate succeeds. Explicit `no merge` narrows transitively; `local-only`, `no commit`, `no push`, and `no PR` also forbid merge. `no cleanup` permits landing and synchronization but forbids branch deletion.
...
Only the native Git/GitHub operations bounded by the loaded skill are supported. Post-create mutation unrelated to ordinal landing, creating an additional checkout or worktree, history rewriting, force, release, configuration changes, credential changes, or destructive recovery are unsupported. Switching the existing checkout back to the original base is authorized only under the landing rules. Existing remote branches or pull requests, rejected pushes, drift, changed remotes, uncertain topology, or dirty worktrees outside that exact bounded interrupted-local-slice recovery gate switch the task to read-only resumption; inspect and report without further mutation.
