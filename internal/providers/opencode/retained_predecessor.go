...
func retainedMarkerName(name string) bool {
	return len(name) == 37 && strings.HasSuffix(name, ".json") && validRetainedOperation(strings.TrimSuffix(name, ".json"))
}

func retainedMarkerAliasName(name string) bool {
	return strings.HasPrefix(name, ".vgxness-retained-") && strings.HasSuffix(name, ".tmp") && len(name) > len(".vgxness-retained-.tmp")
}
