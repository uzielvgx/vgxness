...
func TestActiveV12LifecycleStatusReinstallAndProtection(t *testing.T) {
	...
	service := fakeActivationIntegration(&fakeCodexCLI{fail: map[string]error{}, root: root})
...
func TestIntegrationReinstallsPartialAndPreservesUnrelatedFiles(t *testing.T) {
	options := integration.Options{ConfigDir: filepath.Join(t.TempDir(), "codex")}
	service := fakeActivationIntegration(&fakeCodexCLI{fail: map[string]error{}, root: options.ConfigDir})
