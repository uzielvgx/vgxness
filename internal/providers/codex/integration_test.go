...
func TestIntegrationReinstallsPartialAndPreservesUnrelatedFiles(t *testing.T) {
	...
	mustNoError(t, os.WriteFile(filepath.Join(options.ConfigDir, "unrelated"), []byte("keep"), 0o600))
	mustNoError(t, os.Remove(filepath.Join(options.ConfigDir, "agents", "explore.toml")))
...
func TestIntegrationBlocksDriftAndPendingEvidence(t *testing.T) {
	...
	service := NewIntegration()
	mustInstall(t, service, options)
	mustNoError(t, os.WriteFile(filepath.Join(options.ConfigDir, ".vgxness-pending"), []byte("codex-pending\n"), 0o600))
	...
	mustNoError(t, os.Remove(filepath.Join(options.ConfigDir, ".vgxness-pending")))
	mustNoError(t, os.WriteFile(filepath.Join(options.ConfigDir, "agents", "explore.toml"), []byte("changed"), 0o600))
...
func TestPendingSidecarsBlockOnlyKnownArtifacts(t *testing.T) {
	...
	for _, suffix := range []string{".vgxness-stage", ".vgxness-remove"} {
		...
		mustNoError(t, os.WriteFile(name, []byte("evidence"), 0o600))
		...
		mustNoError(t, os.Remove(name))
	}
	mustNoError(t, os.WriteFile(filepath.Join(options.ConfigDir, ".vgxness-custom"), []byte("keep"), 0o600))
	pending, err := s.ReinstallPending(context.Background(), options)
	require(t, err == nil && !pending)
}
func TestInstallCancellationAndPostLinkFailureRetainRecoveryEvidence(t *testing.T) {
	t.Run("cancellation", func(t *testing.T) {
		...
		defer cancel()
		mustNoError(t, os.Mkdir(options.ConfigDir, 0o700))
		mustNoError(t, os.WriteFile(filepath.Join(options.ConfigDir, "unrelated"), []byte("keep"), 0o600))
		s := NewIntegration()
...
func TestUninstallRejectsReplacedIdentityAndRetainsBackup(t *testing.T) {
	...
	s.checkpoint = func(point, name string) error {
		if point == "before-backup" && name == "AGENTS.md" {
			if err := os.Remove(filepath.Join(options.ConfigDir, name)); err != nil {
				return err
			}
			return os.WriteFile(filepath.Join(options.ConfigDir, name), body, 0o600)
		}
		return nil
	}
	_, err = s.Uninstall(context.Background(), options)
...
func TestUninstallFailedRestoreAndSymlinksRemainRecoveryOrDrift(t *testing.T) {
	...
	t.Run("symlink", func(t *testing.T) {
		...
		mustInstall(t, s, options)
		mustNoError(t, os.Remove(filepath.Join(options.ConfigDir, "agents", "explore.toml")))
		mustNoError(t, os.Symlink("missing", filepath.Join(options.ConfigDir, "agents", "explore.toml")))
	...
	t.Run("agents-dir", func(t *testing.T) {
		...
		for _, item := range pkg.Artifacts {
			if filepath.Dir(item.Path) == "agents" {
				mustNoError(t, os.Remove(filepath.Join(options.ConfigDir, item.Path)))
			}
		}
		mustNoError(t, os.Remove(filepath.Join(options.ConfigDir, "agents")))
		mustNoError(t, os.Symlink("missing", filepath.Join(options.ConfigDir, "agents")))
		got, err := s.Status(context.Background(), options)
		require(t, err == nil && got.State == integration.StateDrifted)
	})
}
...
func mustNoError(t *testing.T, err error) { require(t, err == nil) }
